...
void fft(vector<base>& a, bool inv)
{
    ...
    const long double ang = 2 * M_PIl / n * (inv ? 1 : -1);
    size_t bit, i, j, k;
    ...
    for (i = 0; i < n >> 1; ++i)
        roots[i] = base(cosl(ang * i), sinl(ang * i));
...
void multiply(vector<unsigned __int128>& v,
        const vector<unsigned __int128>& w, unsigned long long p)
{
    size_t n, i, j;
...
void lagrange(vector<unsigned __int128>& h, unsigned long long p)
{
    const size_t d = h.size() - 1;
    size_t i;
    vector<unsigned __int128> fact(4 * d + 2),
        fact_inv(4 * d + 2), inv(4 * d + 2);
    vector<unsigned __int128> f(d + 1), g(4 * d + 2);
...
void squarepoly(vector<unsigned __int128>& poly, unsigned long long p)
{
    size_t len, i;
...
int main()
{
    ...
    if (p < 100000)
    {
        for (i = 2; i <= n; ++i)
            result = result * i % p;
        cout << static_cast<unsigned long long>(result) << '\n';
        return 0;
    }
